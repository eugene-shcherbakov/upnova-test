document.querySelectorAll('.product').forEach((product: Element) => {
  product.addEventListener('click', function (this: HTMLElement) {
    ...
    const productTitle = productClone.querySelector(
      '.product-title'
    ) as HTMLElement;
    ...
    const productDetailView = document.getElementById(
      'productDetailView'
    ) as HTMLElement;
    const productImageContainer = productDetailView.querySelector(
      '.product-image-container'
    ) as HTMLElement;
    const productPriceContainer = productDetailView.querySelector(
      '#detailPrice'
    ) as HTMLElement;
    const backButton = productDetailView.querySelector(
      '#backButton'
    ) as HTMLElement;
    ...
    setTimeout(() => {
      (
        productClone.querySelector('.product-price') as HTMLElement
      ).style.display = 'none';
      (
        productClone.querySelector('.product-description') as HTMLElement
      ).style.display = 'none';
      (
        productClone.querySelector('.product-title') as HTMLElement
      ).style.display = 'none';
    }, 0);
    ...
    setTimeout(() => {
      const updatedProductImageContainerRect =
    ...
    setTimeout(() => {
      ...
      (
        productDetailView.querySelector('#detailTitle') as HTMLElement
      ).innerHTML = productTitleText;
      (
        productDetailView.querySelector('#detailTitle') as HTMLElement
      ).classList.add('animated');
      (
        productDetailView.querySelector('#detailDescription') as HTMLElement
      ).innerHTML = productDescriptionText;
    }, 0);

    // --- Move the product price clone to the center of the product image container ---
    setTimeout(() => {
      const updatedProductPriceContainerRect =
        productPriceContainer.getBoundingClientRect();
